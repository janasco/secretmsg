...
- [ ] **The update feed cannot tell the two tracks apart (blocks public release, not submission):** `/api/app-version` returns one `url` for every client, currently the sideload download page, and the in-app "Update available" dialog opens it verbatim. Sideloaded users are unaffected, but once the Play build is publicly distributed a Play-installed user who falls behind is sent to a page offering an APK Android will refuse to install over the Play copy (`INSTALL_FAILED_UPDATE_INCOMPATIBLE`). Fixing it needs per-track URLs in the feed and a client that can identify its own track — a product decision, since the two tracks never converge; see the per-release runbook step 3 and `apps/mobile-flutter/README.md` ("Two tracks that never merge").
...
3. Bump `android_latest` in the public `/api/app-version` feed to the new release version and deploy that feed with the release. The feed currently exposes `android_latest` at `/opt/secretmsg/secretmsg-private/api/src/index.ts:85-89`; the Flutter app checks it on cold start and when requested from Settings and prompts when its installed version is behind (`apps/mobile-flutter/lib/ritual/update_check.dart:45-66`). Do not publish the AAB while leaving the feed stale. The feed is a **single, track-agnostic `url`** (`https://secretmsg.net/download`) for every client, and the prompt's Update button opens it verbatim (`apps/mobile-flutter/lib/widgets/update_dialog.dart:40-48`); nothing in the client knows which track it is installed from. That is correct while Play is not publicly distributed. Once Play *is* public, a Play-installed user who falls behind is sent to a page offering a sideload APK their device cannot install over the Play copy, and this step becomes a blocker: it needs per-track URLs in the feed plus a client that can identify its own track, both of which are product decisions. The signing and key-rotation background is in `apps/mobile-flutter/README.md` ("Two tracks that never merge"); read it before changing anything about how the AAB is signed.
